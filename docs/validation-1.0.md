...
**Software verification with an explicit mutation waiver; not public-index
publication evidence.** PR #11 merged at `d0c3b863fdf7e87a6243946e97c54f46f43e1db4`.
The implementation was verified at `253729a`. The source baseline is the
...
| --- | --- |
| Fixed-model information states | Correlation, model persistence, ambiguity, visible-history decisions, kernel tampering and finite limit tests passed |
...
| Information comparisons | Matched paid/masked versus no-probe cases, uninformative, irrelevant and overpriced controls passed |
| Composition and revisions | Fresh primitive unfolding, typed interfaces, costs, parent work reservations, preserved histories and tamper tests and critical coverage passed |
...
| Coverage | Local full, CI-selected and release-selected critical suites passed the unchanged aggregate and all 17 per-subsystem gates; metrics below; all six hosted platforms passed at 253729a |
...
| Security and publication hygiene | Local Ruff, strict mypy (134 sources), Bandit, OSV audit, generated references, 225 schemas, 9 fixtures, source/history/archive hygiene, Gitleaks, build and Twine passed; staged/push hygiene and hosted security checks passed at 253729a |
| GitHub, Wiki and PyPI | PR #11 merged after all 12 non-mutation required checks passed; candidate Wiki and waiver disclosure pushed; publication and fresh public-index verification remain separate subsequent records |
...
isolated-layout preparation and later scheduling/waiver tests are included in the
final full rerun at `253729a`. Mutation qualification is explicitly waived.

| Suite | Tests | Statements | Branch destinations | Combined |
| --- | --- | --- | --- | --- |
| Full suite | 946 passed, 3 skipped | 17,772 / 19,021 (93.43%) | 6,164 / 7,156 (86.14%) | 91.44% |
...
quality/security checks passed at `253729a`; a changed head requires fresh checks.
...
96.72% and 97.77%, respectively. Hosted critical checks also passed at `253729a`.
...
The actual Wiki's nine candidate pages were pushed at `30733f9`; the subsequent
waiver disclosure was pushed and verified at `1498c99e5711bc3da7660a570020dfd85689382c`.
Source links, page links, content,
...
It was stopped with its full diagnostic catalogue preserved: 12,951 evaluated,
4,630 incomplete. The score gate correctly rejected that incomplete report even
though the underlying tool returned exit code zero on interruption.
